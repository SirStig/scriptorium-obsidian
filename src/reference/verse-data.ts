 ...
 * Keyed by OSIS book id; index = chapter - 1; value = max verse number.
 *
 * Protestant 66 follow KJV/standard Protestant versification.
 * Deuterocanon uses the most common Catholic/Orthodox numbering;
 * traditions that differ (LXX vs. Vulgate) can override via aliases later.
 *
 * 3 John: 14 (matches KJV; some modern critical editions split v14).
 */

export const VERSE_COUNTS: Record<string, readonly number[]> = {
	// Pentateuch
	Gen: [31, 25, 24, 26, 26, 32, 22, 24, 29, 32, 32, 20, 18, 24, 21, 16, 27, 33, 38, 18, 34, 24, 20, 67, 34, 35, 46, 22, 35, 43, 55, 32, 20, 31, 29, 43, 36, 30, 23, 23, 57, 38, 34, 34, 28, 34, 31, 22, 33, 26],
	...
	// Deuterocanon (active when configureCanon(true, …))
	Tob: [22, 14, 17, 21, 23, 18, 17, 21, 6, 13, 18, 22, 18, 15],
	Jdt: [16, 28, 10, 15, 24, 21, 32, 36, 14, 23, 23, 20, 20, 19, 14, 25],
	Wis: [16, 24, 19, 20, 24, 25, 30, 21, 19, 21, 27, 27, 19, 31, 19, 30, 21, 25, 22],
	Sir: [30, 18, 31, 31, 17, 37, 36, 19, 18, 31, 34, 18, 26, 27, 20, 30, 32, 33, 31, 31, 28, 27, 28, 34, 26, 29, 30, 26, 28, 27, 31, 24, 33, 31, 26, 31, 31, 34, 35, 30, 24, 25, 33, 23, 26, 20, 25, 25, 16, 29, 30],
